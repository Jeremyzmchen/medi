...
def test_evaluate_task_progress_scores_completed_partial_and_pending_tasks() -> None:
    ...
    progress, pending_tasks = evaluate_task_progress(preconsultation_record=record)
...
def test_evaluate_task_progress_tracks_current_medications_and_child_general_condition() -> None:
    ...
    assert record["t2_hpi"]["general_condition"]["urination"]["value"] == "喝水少，尿量正常"
    assert record["t3_background"]["current_medications"]["status"] == "absent"
...
def test_evaluate_task_progress_accepts_triage_values_from_preconsultation_record() -> None:
